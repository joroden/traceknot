...
function runClaim(sessionId, prompt) {
  return new Promise((resolve) => {
    ...
    child.stdin.write(JSON.stringify({ session_id: sessionId ?? "", prompt: prompt ?? "" }));
    child.stdin.end();
  });
}
...
const session = await joinSession({
  hooks: {
    onUserPromptSubmitted: async (input, invocation) => {
      const exitCode = await runClaim(invocation.sessionId, input?.prompt);
      if (exitCode === 2) {
        await session.log(
